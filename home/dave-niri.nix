...

{
    ...
    # Pywalfox: HM's nativeMessagingHosts often doesn't link the manifest (only .keep appears).
    # Install the manifest explicitly so the extension finds the native app.
    home.file.".mozilla/native-messaging-hosts/pywalfox.json".source = pkgs.writeText "pywalfox.json" (builtins.toJSON {
      name = "pywalfox";
